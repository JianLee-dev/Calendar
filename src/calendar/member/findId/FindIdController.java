...
public class FindIdController {
	...
	public void closeFindId() {
		CommonService.close(root);
	}
	
	public void btnFindId() {
		try {
			TextField tmp = (TextField)root.lookup("#txtName");
			name = tmp.getText();
			tmp = (TextField)root.lookup("#txtNum");
			num = Integer.parseInt(tmp.getText());
			
			String result = db.findId(name, num);
			
			if(result != null) {
				//alert로 아이디 알려주기
				CommonService.alert(AlertType.INFORMATION,name +"님의 아이디는 "+result+" 입니다.");
			}else {
